...
$X_1,...,X_n \sim p(x; \theta)$. Queremos estimar $\theta=(\theta_1,...,\theta_k)$
. Un estimador 
$$\hat{\theta} = w(X_1,...,X_n)$$
...
**Ejemplo: Bernoulli**. Supongamos $X_1,...X_n \sim Bernoulli(\theta)$. La función de densidad
correspondiente es $p(x;\theta)=\theta^x(1-\theta)^{1-x}$, por lo que:
...
esto es derivando respecto al vector de parámetros de interés, igualando a cero el sistema 
de ecuaciones resultante, y revisando la segunda derivada para asegurar que se 
encontró un máximo. En el ejemplo este proceso lleva a $\hat{\theta}=S/n$, y 
con $S=12, n = 20$ obtenemos $\hat{\theta}=0.6$

Es muy común recurrir a métodos numéricos (por ejemplo Newton Raphson, BHHH, DFP)
en el caso de R podemos usar las funciones `optim` u `optimize`.
...
curvatura de la función de verosimilitud nos dice que tanta certeza tenemos de
...
sims_boot <- rdply(2000, simBern)
sqrt(1 / 1999 * sum((sims_boot$V1 - p_hat) ^ 2))
...
para esto 
comenzamos creamos una base de datos artificial:
...
plot(base, lty = "solid")
```
Podemos representar el espacio por medio de una expansión lineal en de funciones
base:
...
betas <- rerun(2000, splinesBoot()) %>% reduce(rbind)
...
splines_boot + geom_point(color = "red", alpha = 0.8)
```
...
ggplot(limites) + 
    ...
    geom_point(data = toy, aes(x = x, y = y), color = "red") + 
    stat_function(fun = mu, args = list(betas = beta_hat), color = "blue") +
    labs(x = "", y = "")
...
betas_p <- rerun(2000, splinesBootP()) %>% reduce(rbind)
...
# construimos los intervalos
x_grid <- seq(knots[1], knots[5], 0.02) # creamos un grid para evaluar mu(x)
H <- eval.basis(x_grid, base) # Evaluo la base en el rango
y <- betas %*% t(H) # calculo mu(x*)

y_quant <- t(apply(y, 2, quantile, probs = c(0.025, 0.975))) # calculo intervalos
xy_cuantiles <- data.frame(x = x_grid, y_quant) %>% # creo un data.frame
  gather(cuantil, valor, X2.5., X97.5.)

ggplot(xy_cuantiles) + 
  geom_line(aes(x = x, y = valor, group = cuantil), color = "darkgray") +
  geom_point(data = toy, aes(x = x, y = y), color = "red") + 
  stat_function(fun = mu, args = list(betas = beta_hat), color = "blue") 
```
...
```{r, fig.width = 4}
seMu <- function(x, lim = "sup"){ # lim es un string, si sup regresa límite superior
  # calculo h(x)
  h <- eval.basis(x, base)
  # calcilo se_hat(x)
...
ggplot(toy) +
  geom_point(color = "red", aes(x = x, y = y)) + 
...
$\theta = e^{\mu}$, crea una base de datos usando $\mu=5$ que consista de $n=100$ observaciones.
