...
flights <- read_csv("datos/flights.csv")
flights

weather <- read_csv("datos/weather.csv")
weather 

planes <- read_csv("datos/planes.csv")
planes

airports <- read_csv("datos/airports.csv")
airports
```
...
cada grupo de interés y aplicar las funciones, sin embargo la aplicación de ciclos 
`for` desemboca en código difícil de entender por lo que preferimos trabajar 
con funciones creadas para estas tareas, usaremos el paquete `dplyr` que además
de ser más claro suele ser más veloz.
...
```{r}
billboard <- read_csv("datos/billboard.csv")
billboard
```
...
```{r}
tb <- read.csv("datos/tb.csv") %>% tbl_df()
tb
```
...
```{r}
clima <- read_delim("datos/clima.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
```
